...
#Subtracts the magnet repelling's location by the repelled magnet's location to find the direct x z difference in coordinates
...
#Duplicating the x z values so we have a copy of them for later (all store the dx dz, do not get confused)
...
#Making each magnet_distance inverted, then multiplying it by the x z directions to get the correct amount of power
...
execute as @e[type=item,tag=repelled] store result entity @s Motion[0] double 0.0006 run scoreboard players get @s magnet_dx
execute as @e[type=item,tag=repelled] store result entity @s Motion[2] double 0.0006 run scoreboard players get @s magnet_dz
...
##Floorcrafting Magnet
#Crafts the item recursively until can no longer be crafted; plays the particles and sounds; removes the magnetCrafted tag
...
execute as @e[tag=magnetCrafted] at @s run particle minecraft:flame ~ ~ ~ 0 0 0 0.1 30
execute as @e[tag=magnetCrafted] at @s run playsound minecraft:entity.blaze.death master @a ~ ~ ~
execute as @e[tag=magnetCrafted] at @s run playsound minecraft:block.anvil.use master @a ~ ~ ~ 0.7
tag @e[tag=magnetCrafted] remove magnetCrafted