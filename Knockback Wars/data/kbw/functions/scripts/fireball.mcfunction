...
execute at @s anchored eyes run summon minecraft:fireball ^ ^ ^ {Tags:["pyroFireball"],ExplosionPower:2}
...
execute store result entity @e[type=minecraft:fireball,tag=pyroFireball,limit=1,sort=nearest] Motion[0] double 0.001 run scoreboard players get @s calculation
...
#Killing the temporary marker (even though it dies in 1 tick by itself)
