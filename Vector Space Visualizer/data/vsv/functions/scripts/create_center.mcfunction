#Creates a center
kill @e[tag=vsvAxis,type=armor_stand]
kill @e[tag=vsvAxis,type=block_display]

execute store result score %origin_x values run data get entity @s Pos[0] 1
execute store result score %origin_y values run data get entity @s Pos[1] 1
execute store result score %origin_z values run data get entity @s Pos[2] 1
execute at @s align xyz run summon armor_stand ~ ~ ~ {NoGravity:1b,Silent:1b,Invulnerable:1b,Marker:1b,Invisible:1b,NoBasePlate:1b,Tags:["vsvAxis"]}
...
execute if score @s center matches 1..2 at @s align xyz as @e[type=armor_stand,limit=1,sort=nearest,tag=vsvAxis] run scoreboard players set @s values 20
