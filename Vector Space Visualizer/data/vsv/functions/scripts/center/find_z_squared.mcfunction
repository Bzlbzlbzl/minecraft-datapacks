...
execute if score %dist values matches 0 store result score %z_sq values run data get entity @e[type=area_effect_cloud,limit=1,tag=vsvMarker] Pos[1] 1
execute if score %dist values matches 0 run kill @e[tag=vsvMarker]
execute if score %dist values matches 0 run scoreboard players operation %z_sq values -= %origin_z values
