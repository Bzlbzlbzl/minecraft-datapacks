...
execute positioned ^ ^ ^0.1 if block ~ ~ ~ water[level=0] align xyz as @e[tag=aetherCheck,distance=..0.01,tag=!aetherFailed,tag=!aetherNorth,tag=!aetherSouth,tag=!aetherEast,tag=!aetherWest,limit=1,sort=nearest] run function why:aether/check12
execute if entity @s[tag=aetherPlayer] run tag @s remove aetherPlayer
execute if score @s aether_water matches 1.. positioned ^ ^ ^0.1 run function why:aether/find_water