...
#summons slime to bobber after 3 ticks and keeps it attached   #armor stand is simply to hide slime name upon spawn
execute as @e[type=minecraft:fishing_bobber,tag=!summoned,scores={rodDelayTimer=3..}] at @s run summon minecraft:slime ~ ~-1 ~ {Size:1b,Tags:["rodDamage"],Silent:1,Invulnerable:1,Attributes:[{Name:"generic.attack_damage",Base:000.1d}],CustomName:'{"text":"Rod Damage"}',ActiveEffects:[{Id:14,Amplifier:0,Duration:100000,ShowParticles:0b}],Passengers:[{id:armor_stand,Invisible:1b,Marker:1b,Tags:["toKill"]}]}
...
#kills bobber once player delay is over, or if the bobber has hit the ground, or if bobber is above water
...
#removing the slime if the bobber disappears, and killing the armor stand
...
kill @e[tag=toKill,type=armor_stand]