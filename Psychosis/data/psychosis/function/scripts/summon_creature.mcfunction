...
#Summoning skeleton
execute as @e[tag=psyMarker] at @s run summon skeleton ~ ~ ~ {Silent:1b,Invulnerable:1b,DeathLootTable:"minecraft:empty",NoAI:1b,Tags:["psyCreature","psySpawned"],CanPickUpLoot:0b,PersistenceRequired:1b}
...
execute if score @s psychosis matches 72001.. as @e[tag=psySpawned,limit=1] at @s run summon enderman ~ ~ ~2 {Silent:1b,Invulnerable:1b,DeathLootTable:"minecraft:empty",PersistenceRequired:1b,NoAI:1b,CanPickUpLoot:0b,Tags:["psyCreature","psySpawned"],carriedBlockState:{Name:"minecraft:structure_void"}}
...
execute as @e[tag=psySpawned,type=enderman,limit=1] at @s rotated ~60 0 positioned ^ ^-0.7 ^0.8 run summon armor_stand ~ ~ ~ {Silent:1b,Invulnerable:1b,Marker:1b,Invisible:1b,Tags:["psyHead","psySpawned"],equipment:{head:{id:"minecraft:player_head",count:1,components:{"minecraft:profile":{name:"Catxolotl"}}}}}
