...
execute as @e[tag=psyMarker,limit=1] at @s if entity @e[type=minecraft:player,gamemode=!spectator,distance=..8,scores={psychosis=72000..}] run scoreboard players set %spread_success psychosis 0
execute as @e[tag=psyMarker,limit=1] at @s if entity @e[type=minecraft:player,gamemode=!spectator,distance=..20,scores={psychosis=..71999}] run scoreboard players set %spread_success psychosis 0
