...
execute as @a[team=p-prot,gamemode=!spectator] at @s as @e[type=cow,distance=..8,tag=!pProt,nbt={Age:0}] run data merge entity @s {CustomNameVisible:1b,CustomName:'{"text":"Cow Protector","color":"green","bold":true}',ActiveEffects:[{Id:10b,Amplifier:0b,Duration:2147483647}],Attributes:[{Name:generic.armor,Base:12}],Team:"p-prot"}
execute as @a[team=p-prot,gamemode=!spectator] at @s as @e[type=cow,distance=..8,tag=!pProt,nbt={Age:0}] run function p-prot:assign_next
execute as @a[team=p-prot,gamemode=!spectator] at @s run tag @e[type=cow,distance=..8,tag=!pProt,nbt={Age:0}] add pProt
